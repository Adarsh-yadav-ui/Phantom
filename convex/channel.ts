...
export const createChannel = mutation({
  ...
  handler: async (
    ctx,
    { cultId, channelName, channelDesc, joinCode, type, visibility },
  ) => {
    ...
    const newChannelId = await ctx.db.insert("channel", {
      cultId,
      channelName,
      channelDesc,
      joinCode,
    ...
    // Auto-enroll all existing cult members into the new public channel
    if (visibility === "public") {
      const cultMembers = await ctx.db
        .query("cultMembers")
        .withIndex("byCultId", (q: any) => q.eq("cultId", cultId))
        .collect();

      await Promise.all(
        cultMembers.map((member) =>
...
export const updateChannel = mutation({
  ...
  handler: async (ctx, { channelId, channelName, channelDesc, visibility }) => {
    const identity = await ctx.auth.getUserIdentity();
    if (!identity) throw new Error("Not authenticated");
